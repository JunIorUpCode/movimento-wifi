 ...
 * Hook useAuth
 * 
 * Gerencia o estado de autenticação do usuário tenant.
    ...
    const loadAuth = () => {
      try {
        ...
        if (token && userStr) {
          ...
          // Verifica se o usuário é tenant
          if (user.role !== 'tenant') {
            // Limpa dados se não for tenant
  ...
  const login = useCallback(async (credentials: LoginCredentials) => {
    try {
      ...
      if (response.user.role !== 'tenant') {
        throw new Error('Acesso negado. Apenas clientes podem acessar este painel.')
      }
